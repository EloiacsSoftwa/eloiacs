...
import VehicleInvoiceTableBody from "./VehicleInvoiceTableBody";
import { connect } from "react-redux";


const VehicleInvoiceForm = (props) => {
  const tableHeader = [
    "Vehicle",
    "Description",
    // "Quantity",
  ...
  const handleDeleteRow = (id) => {
    setVehicleInvoiceData(vehicleInvoiceData.filter((item) => item.id !== id));
    setRowCount(rowCount - 1);
  };
  ...
  }, [globalDiscountValue, description]);
      ...
      <Container fluid className="mt-1">
        ...
        <div style={{ paddingLeft: 50, paddingRight: 50 }}>
          <Table hover size="sm" bordered responsive>
            ...
            <VehicleInvoiceTableBody
              products={props.vehicle.vehicleList}
...
export default connect(mapsToProps)(VehicleInvoiceForm);
